...
BOOL llama_get_model_type(
    ...
    NSError **outError) {
    try {
        llama_context *lctx = new llama_context;

        std::unique_ptr<llama_model_loader> ml(new llama_model_loader(fname, false, true));
...

        {
            switch (hparams.n_layer) {
                case 32: model_type = e_model::MODEL_7B; break;
                case 40: model_type = e_model::MODEL_13B; break;
                case 60: model_type = e_model::MODEL_30B; break;
                case 80: model_type = e_model::MODEL_65B; break;
            }
        }
        ...
        if (outError) {
            ...
            if (!err.empty()) {
                errorDescription = [NSString stringWithCString:err.c_str() encoding:NSUTF8StringEncoding];
            }
            *outError = makeCameLLMErrorWithUnderlyingError(_CameLLMErrorCodeFailedToGetModelType, @"Couldn't determine model type", makeCameLLMError(_CameLLMErrorCodeGeneralInternalLoadFailure, errorDescription));
        }
        return NO;
    }
}
...
static BOOL llama_model_load_internal(
...
static BOOL llama_model_load(
    ...
    try {
        llama_model_load_internal(fname, lctx, n_ctx, memory_type, use_mmap, use_mlock,
                                  vocab_only, progress_callback, progress_callback_user_data, outError);
        return YES;
    } catch (const std::string & err) {
        if (outError) {
            *outError = makeFailedToLoadModelErrorWithUnderlyingError(makeCameLLMError(_CameLLMErrorCodeGeneralInternalLoadFailure, [NSString stringWithCString:err.c_str() encoding:NSUTF8StringEncoding]));
        }
        return NO;
    }
}
    ...
    for (llama_load_tensor & tensor : model_loader->tensors_map.tensors) {
        ...
        if (!quantize) {
            ...
            switch (new_type) {
                ...
                default:
                  throw format("unsupported type %d", new_type);
            }
...
struct llama_context * llama_init_from_file(const char * path_model, struct llama_context_params params, NSError **outError) {
    ggml_time_init();
    ...
    if (!llama_model_load(path_model, *ctx, params.n_ctx, memory_type,
                          ...
                          params.progress_callback, params.progress_callback_user_data, outError)) {
//        fprintf(stderr, "%s: failed to load model\n", __func__);
    ...
    if (!params.vocab_only) {
        if (!kv_cache_init(ctx->model.hparams, ctx->model.kv_self, memory_type, ctx->model.hparams.n_ctx)) {
            if (outError) {
                *outError = makeFailedToLoadModelErrorWithUnderlyingError(makeCameLLMError(_CameLLMErrorCodeGeneralInternalLoadFailure, [NSString stringWithFormat:@"kv_cache_init() failed for self-attention cache"]));
            }
            llama_free(ctx);
            return nullptr;
        }
...
BOOL llama_model_quantize(
  ...
  enum llama_ftype   ftype,
    ...
    try {
        ...
        if (outError) {
            *outError = makeError(_CameLLMLlamaErrorDomain, _CameLLMLlamaErrorCodeFailedToQuantize, [NSString stringWithCString:err.c_str() encoding:NSUTF8StringEncoding], nil);
        }
        return NO;
    }
}
...
int llama_eval(
        struct llama_context * ctx,
           const llama_token * tokens,
                         ...
                         int   n_threads,
                     NSError **outError) {
    if (!llama_eval_internal(*ctx, tokens, n_tokens, n_past, n_threads, outError)) {
        return 1;
    }
...
int llama_tokenize(
    ...
    if (n_max_tokens < (int) res.size()) {
        if (outError) {
            *outError = makeFailedToPredictErrorWithUnderlyingError(makeCameLLMError(_CameLLMErrorCodeGeneralInternalPredictionFailure, @"too many tokens"));
        }
        return -((int) res.size());
    }
